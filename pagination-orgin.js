$.fn.extend({
	pagination(_object_cfg){
  	...
  	_.fun = {
  		...
  		countPage(){
				let int_allPage = Math.ceil(_.cfg.count/_.cfg.numsPerPage);//一共有几页
				let int_allowPage = _.cfg.allowPage;//允许显示几页
				let int_currentPage = parseInt(_.cfg.currentPage)//当前页
				let int_init = 1;//初始化页数
				let int_leftNum = 0==int_allowPage%2?int_allowPage/2:((int_allowPage-1)/2);
				let int_rightNum = (int_allowPage-1)-int_leftNum;//3
				let int_lastNum = int_allPage-int_rightNum;
